...
describe('Starting tests for hackbo-angular', function() {
  ...
  it('website title should be hackbo-angular', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('hackbo-angular');
    })
  });

  it('network-name should be hackbo@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hackbo@0.0.1.bna');
    });
  });

  it('navbar-brand should be hackbo-angular',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('hackbo-angular');
    });
  });
